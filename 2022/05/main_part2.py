...
def execute_instr(state, instr):
    ...
    state = do_move(state, repeat, fr, to)
    print(state)
    return state


def do_move(state, repeat, fr, to):
    chars = []
    for _ in range(repeat):
        chars.append(state[fr-1].pop())
    state[to-1].extend(reversed(chars))
    return state
