...
def recurse_replace(reactions, nums, level=0):
    print(nums)
    level_names = []
    new = []
    for k, v in nums:
    ...
    if len(final.keys()) > 1:
        new, final = recurse_replace(reactions, list(new), level + 1)
...
def parse_reactions(text):
    ...
    set_levels(reactions)
    return reactions


def set_levels(reactions, start='FUEL', depth=0):
    if start not in reactions:
        return
    r = reactions[start]
    r.level = max(depth, r.level)
    for i in r.inputs:
        set_levels(reactions, i.name, depth + 1)


def get_total_ore(text):
    reactions = parse_reactions(text)
    return calc_total_ore(reactions, 1)


def calc_total_ore(reactions, fuel):
    # nums = [('FUEL', 2267000)]
    nums = [('FUEL', fuel)]
    nums, final = recurse_replace(reactions, list(nums))
    print(final)
...
def max_fuel_from_ore_value(text, ore):
    ...
    for i in range(2267000, 2268000):
        total_ore = calc_total_ore(reactions, i)
        ratio = (total_ore / ore)
        print(ratio)
        if ratio > 1:
            return i - 1
