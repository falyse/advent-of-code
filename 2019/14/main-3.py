...
def recurse(start, need=1):
    ...
    for r in reacts:
        if r.output.name == start:
            ...
            print('  ', r)
    if len(matches) == 0:
        print('     leaf adding', need, start)
        if start not in amts:
            amts[start] = 0
        amts[start] += need
    for m in matches:
        print('     adding', m.output.quantity, m.output.name)
        if m.output.name not in amts:
            amts[m.output.name] = 0
        amts[m.output.name] += need * m.output.quantity
        for i in m.inputs:
            extra = m.output.quantity - need
            print('asdf1', start, i.name, i.quantity, m.output.name)
            if i.name in extras and extras[i.name] + extra >= i.quantity:
                print('   used extra', i.quantity, i.name)
                extras[i.name] -= i.quantity
            else:
                print('asdf0', i.name, i.quantity, need, m.output.quantity)
                # print('     adding', i.quantity * (need / m.output.quantity), i.name)
            if extra > 0:
                print('   extra', extra, m.output.quantity, i.name)
            ...
            print(amts)
            print(extras)
            recurse(i.name, i.quantity * (need / m.output.quantity))
