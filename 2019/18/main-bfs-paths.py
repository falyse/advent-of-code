...
def solve_maze(input):
    ...
    return navigate_maze(maze, all_paths)
...
def navigate_maze(maze, all_paths):
    # BFS to find all the keys
    maze = maze.copy()
    visited = []
    queue = [{'key': '@', 'steps': 0}]
    while len(queue) > 0:
        ...
        if current['key'] not in visited:
            visited.append(key)
            steps = current['steps']
            print('At key', key, 'in', steps, 'steps')
            print('  visited', visited)

            # Add newly opened paths
            open_keys = get_open_path_keys(all_paths, key, visited)
            print('  open keys', open_keys)
            for next_key, dist in open_keys:
                if next_key not in visited:
                    print('    next_key', next_key)
                    queue.append({'key': next_key, 'steps': steps+dist})
    print('steps', steps)


def get_open_path_keys(all_paths, src, visited):
    open_keys = []
    for path, v in all_paths.items():
        if path[0] == src:
            if all([x in visited for x in v['needs']]):
                open_keys.append((path[1], v['steps']))
    return open_keys
